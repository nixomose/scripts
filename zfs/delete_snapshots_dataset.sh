...
usage="$(basename "$0") -o DATASET -s SECONDS -m MINUTES -h HOURS -d DAYS -p GREP_PATTERN [-t]\n
...
snapshots=`zfs list -H -t snapshot -o name,creation -s creation $dataset | grep $pattern`

if [[ -z $snapshots ]]; then
  echo "no snapshots found for pattern $pattern on dataset $dataset"
  exit 0
fi
