...
# get a list of all of the days in the past 2 weeks, keep every snapshot 
for i in {0..8}; do ((keep[$(date +%Y%m%d -d "-$i day")]++)); done
...
# so now we have all the snapshots for the recent 2 weeks we want to keep

# get a list of all of the wenesdays in the past year
